...
if [ $CPU_CORE ]; then
    if [ $MEM_UTIL ]; then
        ...
        /usr/local/bin/jenkins-java -c $CPU_UTIL -n $CPU_CORE -m ${MemUsage}MB
    else
        /usr/local/bin/jenkins-java -c $CPU_UTIL -n $CPU_CORE
    fi
else
    if [ $MEM_UTIL ]; then
        MemUsage=$(($MemTotal / 100 * $MEM_UTIL))
        /usr/local/bin/jenkins-java -c $CPU_UTIL -r curve -m ${MemUsage}MB
    else
        /usr/local/bin/jenkins-java -c $CPU_UTIL -r curve
    fi
fi
